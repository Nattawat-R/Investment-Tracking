...
import AddUserTransactionForm from "@/components/add-user-transaction-form"
...
import {
  ...
  getUserProfile,
  updateUserProfile,
} from "@/app/actions/user-transactions"
import { apiService } from "@/lib/optimized-api-service"
import type { Transaction, UserProfile } from "@/lib/supabase-client"
...
export default function AuthenticatedInvestmentDashboard() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [profileData, transactionsData, holdingsData] = await Promise.all([
      ...
      // Set display currency from user profile
      if (profileData?.preferred_currency) {
        setDisplayCurrency(profileData.preferred_currency)
      }
      ...
      if (holdingsData.length > 0) {
        ...
        // Then fetch fresh market prices using optimized API service
        const symbols = holdingsData.map((h) => h.symbol)
        console.log(`🔄 User ${user.email} fetching fresh prices for ${symbols.length} symbols...`)

        try {
          const quotes = await apiService.queueRequest(user.id, symbols, "high")
          ...
          console.log(`✅ User ${user.email} updated ${quotes.length} fresh prices`)
        } catch (error) {
  ...
  const handleDeleteTransaction = async () => {
    ...
    try {
      const result = await deleteUserTransaction(transactionToDelete.id, user.id)
      if (result.success) {
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      const result = await updateUserTransaction(transactionToEdit.id, user.id, editFormData)
      if (result.success) {
  ...
  const handleCurrencyChange = async (newCurrency: "USD" | "THB") => {
    setDisplayCurrency(newCurrency)

    // Update user profile with new preferred currency
    if (user) {
      await updateUserProfile(user.id, { preferred_currency: newCurrency })
    }
  }
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex-1">
            ...
            <p className="text-sm text-gray-400 mt-1 flex flex-col sm:flex-row sm:items-center gap-2">
              <span>Your personal investment tracking dashboard</span>
              {exchangeRate && (
          ...
          <div className="flex gap-2 flex-wrap sm:flex-nowrap">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="bg-gray-900 border-gray-700" align="end">
                ...
                <DropdownMenuLabel className="text-xs text-gray-400 font-normal">{user.email}</DropdownMenuLabel>
